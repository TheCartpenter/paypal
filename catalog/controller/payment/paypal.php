...
class PayPal extends \Opencart\System\Engine\Controller {
				...
				if ($paypal_order_info && !$this->error) {
					...
					if (empty($paypal_order_info['payer']['name']) && empty($paypal_order_info['purchase_units'][0]['shipping']['address']) && !empty($paypal_order_info['payment_source'])) {
						foreach ($paypal_order_info['payment_source'] as $payment_source) {
							$this->session->data['customer']['firstname'] = ($payment_source['card']['name'] ?? '');

							$this->session->data['payment_address']['firstname'] = ($payment_source['card']['name'] ?? '');
							$this->session->data['payment_address']['address_1'] = ($payment_source['card']['billing_address']['address_line_1'] ?? '');
							$this->session->data['payment_address']['city'] = ($payment_source['card']['billing_address']['admin_area_2'] ?? '');
							$this->session->data['payment_address']['postcode'] = ($payment_source['card']['billing_address']['postal_code'] ?? '');
	...
	/**
	 * Extension Get Extensions By Type After
	 *
	 * @param string               $route
	 * @param array<string, mixed> $data
	 * @param mixed                $output
	 ...
	 * @return void
	 */
	public function extension_get_extensions_by_type_after(string &$route, array &$data, &$output): void {
	...
	/**
	 * Extension Get Extension By Code After
	 *
	 * @param string               $route
	 * @param array<string, mixed> $data
	 * @param mixed                $output
	 */
	public function extension_get_extension_by_code_after(string &$route, array &$data, &$output): void {
		if ($this->config->get('payment_paypal_status') && $this->config->get('payment_paypal_client_id') && $this->config->get('payment_paypal_secret')) {
			$type = $data[0];
			$code = $data[1];
